...
if (!isset($_GET['lang']) || !isset($_GET['id'])) {header('Location: '.str_replace('/es/','/',$url_prefix)); exit(); }
...
if ($nr == 0) {header('Location: '.str_replace('/es/','/',$url_prefix)); exit(); }
$sql = mysql_fetch_row($sql);
if ($sql[1] != 'yes') {header('Location: '.str_replace('/es/','/',$url_prefix)); exit(); }
...
ob_clean();
$pdf->SetAuthor($name);
$pdf->Output("Curriculum Vitae - $name - ".ucfirst($_GET['lang']).".pdf","D");

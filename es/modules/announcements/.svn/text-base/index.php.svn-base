...
starttabpane('announcements-pane');
starttabpage($translate->_("announcements"));
...
<input name="back" type="button" onClick="javascript: window.location='announcements';" value="<?php echo $lang['back']; ?>" /> 
...
ob_clean();
redirect ('cms/announcements?id='.$_GET['id'].'&action=edit&disabled=true');
exit();
...
echo "<form name=\"updateform\" method=\"post\" "
 ."action=\"postannouncements?action=update&amp;id=".$nr."&amp;page=".$_GET['page']."\">";

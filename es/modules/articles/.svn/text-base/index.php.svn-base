...
starttabpane('articles-pane');
starttabpage($translate->_("articles"));
...
if (!isset($_GET['edit'])) { $_GET['edit'] = "default"; }
$edit = $_GET['edit'];
...
<h1><?php echo $lang['cat']."</h1>";

$rows = $db->fetchAll(
"SELECT * FROM ".$config->database->dbprefix
."_content_cat WHERE id > :id ORDER BY id DESC;", 
array('id' => 0)
);
...
echo "<form name=\"updateform\" method=\"post\" "
 ."action=\"postarticles?action=update&amp;id=".$nr."&amp;page=".$_GET['page']."\">";
...
<table width="100%" border="0" cellspacing="0" cellpadding="5" class="list-table">
  <tr>
    ...
    <td width="5%"><?php echo $lang['id']; ?></td>
	<td width="90%"></td>
  </tr>
...
<input type="button" onClick="javascript: window.location='?action=cat&edit=new';" name="new" value="<?php echo $lang['new']; ?>" />
<?php if ($nr > 0) { ?>
...
."_content WHERE id != :sub AND type = :type AND module = :mod ORDER BY created_date DESC;", 
array('type' => 'content','sub' => $_GET['id'],'mod' => 2)
);
...
<input name="back" type="button" onClick="javascript: window.location='articles';" value="<?php echo $lang['back']; ?>" /> 
...
ob_clean();
redirect ('cms/articles?id='.$_GET['id'].'&action=edit&disabled=true');
exit();
...
$rows = $db->fetchAll(
"SELECT id, title, sub_of FROM ".$config->database->dbprefix
."_content WHERE type = :type AND module = :mod ORDER BY created_date DESC;", 
array('type' => 'content','mod' => 2)
);
...
$rows = $db->fetchAll(
"SELECT * FROM ".$config->database->dbprefix
."_content WHERE id > :id AND type = :type ORDER BY public_date DESC;", 
array('id' => 0, 'type' => 'articles')
);

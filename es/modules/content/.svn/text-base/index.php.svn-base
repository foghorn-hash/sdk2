...
starttabpane('content-pane');
starttabpage($translate->_("content"));
...
<?php 
$rows = $db->fetchAll(
"SELECT id, title, sub_of FROM ".$config->database->dbprefix
."_content WHERE id != :sub AND type = :type ORDER BY created_date DESC;", 
array('type' => 'content','sub' => $_GET['id'])
);
...
<?php 
  foreach ($rows as $field_index => $field_name)
   ...
   {
   echo "<option value=\"".$field_name['id']."\""; 
   if ($row['sub_of'] == $field_name['id']) {echo " selected";} 
   echo ">".$field_name['title']."</option>";
...
."_templates WHERE id > :id ORDER BY folder ASC;", 
array('id' => 0)
);
  ...
  foreach ($rows as $field_index => $field_name)
   ...
   {
   echo "<option value=\"".$field_name['id']."\""; 
   if ($row['temp'] == $field_name['id']) {echo " selected";} 
   echo ">".$field_name['folder']."</option>";
...
$rows = $db->fetchAll(
"SELECT * FROM ".$config->database->dbprefix
."_content_mod WHERE id > :id ORDER BY lang_var ASC;", 
array('id' => 0)
);
  ...
  foreach ($rows as $field_index => $field_name)
   ...
   {
   echo "<option value=\"".$field_name['id']."\""; 
   if ($row['module'] == $field_name['id']) {echo " selected";} 
   echo ">".$field_name['lang_var']."</option>";
...
fckeditor($phpNative['lang'][$langname]['content']);

?>

<input name="submit" type="submit" value="<?php echo $lang['save']; ?>" /> 

<input name="back" type="button" onClick="javascript: window.location='content';" value="<?php echo $lang['back']; ?>" /> 
...
$set = array (
	...
	'sub_of' => strip_tags(stripslashes($_POST['subof'])),
	'temp' => strip_tags(stripslashes($_POST['tmp'])),
	'module' => strip_tags(stripslashes($_POST['modules'])),
);
...
ob_clean();
redirect ('cms/content?id='.$_GET['id'].'&action=edit&disabled=true');
exit();
...
$rows = $db->fetchAll(
"SELECT * FROM ".$config->database->dbprefix
."_content WHERE id > :id AND type = :type ORDER BY order_id ASC, sub_of ASC;", 
array('id' => 0, 'type' => 'content')
);
...
echo "<form name=\"updateform\" method=\"post\" "
 ."action=\"".$url_prefix."cms/postcontent?action=update&amp;id=".$nr."&amp;page=".$_GET['page']."\">";
...
<table width="100%" border="0" cellspacing="0" cellpadding="5" class="list-table">
  <tr>
    ...
    <td width="15%"><?php echo $lang['title']; ?></td>
...
<?php
  ...
  foreach ($rows as $field_index => $field_name)
   ...
   ."\n<td class=\"$class\"><input type=\"text\" size=\"1\" name=\"order".$i."\" value=\"".$field_name['order_id']."\" /></td>";
   $result = $db->fetchRow(
...
<input type="submit" name="od" value="%" />
